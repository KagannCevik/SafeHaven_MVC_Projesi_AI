...
namespace Project7DayAndNight.Controllers
{
    public class SocialController : Controller
    {
        ...
        public async Task<ActionResult> LinkedinFollower()
        {
            string username = "muratyucedag"; 
            ...
            using (var client = new HttpClient())
            {
                ...
                using (var response = await client.SendAsync(request))
                {
                    ...
                    try
                    {
                        ...
                        var viewModel = new LınkedinViewModel
                        {
                            Username = username,
                        ...
                        return View("LinkedinFollower", viewModel); // View'e gönder
                    }
                    catch (Exception)
                    {
                        var fallbackModel = new LınkedinViewModel
                        {
                            Username = username,
                            FollowersCount = 0,
                        ...
                        return View("LinkedinFollower", fallbackModel);
                    }
                }
            }
        }
